...
// TODO should change the way audioURL is updated/located
export default function Book({
  ...
  function checkCurrAnswer() {
    for (
      let idx = 0;
      idx <
      choiceStatus["sections"][currStatus.section]["questions"][
        currStatus.question
      ]["options"].length;
      idx++
    ) {
      if (
        book.sections[currStatus.section].questions[currStatus.question]
          .options[idx].flag
      ) {
        if (
          choiceStatus["sections"][currStatus.section]["questions"][
  ...
  function handleNextClick() {
    ...
    if (
      currStatus.question ===
      book.sections[currStatus.section].questions.length - 1
    ) {
      if (currStatus.question !== -1) {
        //there is no question for the section
  ...
  const saveBookProgress = async () => {
    try {
      const response = await fetch(
        `${backendAddress}books/book_progress/${id}/${user_id}/set/`,
        {
          method: "POST",
          headers: {
  ...
  const fetchNewAudio = async (newStatus) => {
    if (String(newStatus) === cannot_move_next) {
      try {
        ...
        const response = await axios.get(
          `${backendAddress}reading_app/audio/answer_before_next`,
          {
            responseType: "blob",
          }
        );
    ...
    try {
      // Fetch the new audio
      const response = await axios.get(
        `${backendAddress}books/audio/${id}/section_${
          newStatus.section + 1
        }/question_${newStatus.question + 1}/choice_${newStatus.choice + 1}`,
        {
          responseType: "blob",
        }
      );
  ...
  useEffect(() => {
    ...
    const fetchBookProgressTemplate = async () => {
      try {
        const response = await fetch(
          `${backendAddress}books/book_progress_template/${id}/`
        );
    ...
    <View>
      ...
      {currStatus.section === book.sections.length ? (
        ...
        <Section
          ...
          currSection={currStatus.section}
          currQuestion={currStatus.question}
